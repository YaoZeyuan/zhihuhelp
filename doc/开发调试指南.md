...
2.  包管理器使用 pnpm 11.x。根目录和 `client` 的 `package.json` 都声明了 `packageManager: pnpm@11.5.0`，CI 也会通过 Corepack 激活同一版本。
3.  Windows GUI/打包环境建议安装 Visual Studio 2022 C++ 构建工具和 Python 3，用于 `sqlite3`、`sharp` 等原生依赖在必要时重建。
4.  Electron GUI 需要可视化桌面环境；纯命令行抓取和生成可以只跑 Ace 命令。
...
| 路径     | 职责                                                                 |
| -------- | -------------------------------------------------------------------- |
| `.`      | Electron 主进程、Ace 命令、知乎接口、SQLite 数据模型、EPUB/HTML 生成 |
...
| `pnpm ace Init:Env`          | 初始化目录和 SQLite 表结构                            |
| `pnpm ace Fetch:Customer`    | 按 `config.json` 抓取知乎数据并写入数据库             |
| `pnpm ace Generate:Customer` | 按 `config.json` 从数据库生成 HTML/EPUB               |
...
1.  执行 `pnpm build`，确保 `dist/ace.js` 已生成。
2.  执行 `pnpm ace Init:Env`，创建缓存目录、输出目录和 SQLite 表。
3.  准备 `config.json`。可以参考 `demo.config.json`、`demo.customer_task_config.json` 和 `demo.task_config_list.json`。
4.  执行 `pnpm ace Fetch:Customer` 抓取数据。
5.  执行 `pnpm ace Generate:Customer` 生成电子书。

开发时可以用一个终端执行 `pnpm watch`，另一个终端反复运行 Ace 命令。
...
4.  在 GUI 中登录知乎、配置任务并开始执行。GUI 会收集 cookie，写入 `config.json`，然后依次触发 `Init:Env`、`Fetch:Customer`、`Generate:Customer`。

VSCode 中可以直接使用 `Debug Electron` 配置。`pnpm ace Init:Env` 也已经配置为 Node Terminal 调试项。
...
`src/ace.ts` 是命令行入口，会先生成 Ace manifest，再初始化环境，最后执行用户传入的命令。主要命令在 `src/command`：

| 目录或文件                         | 职责                                                     |
| ---------------------------------- | -------------------------------------------------------- |
| `src/command/init_env.ts`          | 初始化缓存目录、输出目录和 SQLite 表                     |
| `src/command/fetch/customer.ts`    | 读取任务配置，按类型合并抓取任务并派发到 `src/api/batch` |
| `src/command/generate/customer.ts` | 从数据库读取内容，排序、分卷，并生成 HTML/EPUB           |
| `src/command/base.ts`              | 命令基类，统一日志和异常包装                             |
...
生成流程先把数据库记录整理为“用户/话题/收藏夹/专栏/混合类型”等单元，再把单元拆成“问题/文章/想法”页面。`src/command/generate/library/html_render` 使用 React 模板生成 HTML，`src/command/generate/library/epub_generator.ts` 和 `src/library/epub` 负责 EPUB 资源、目录和打包。
