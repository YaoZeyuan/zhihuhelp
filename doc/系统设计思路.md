...
2.  所以, 所有导出统一使用 default 导出, 而非具名导出.
3.  所有单词统一使用单数, 避免名词复数拼写不一致问题
4.  文件名使用下划线分割, 以适配大小写不敏感的文件系统
5.  使用具名对象形式传递参数, 而非数组形式. 适配部分参数为非必填的情况
6.  涉及时间的单位, 统一使用 ms, 变量名后加\_ms 后缀, 方便增加可读性
