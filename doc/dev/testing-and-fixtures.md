...
| U13  | 输出历史      | 仅扫描最近 5 个自然日中 `success/partial_success` 且带书籍级 `outputPath` 的 `output.created`、按目录规范化去重、倒序、损坏行容错、忽略无 `outputPath` 的旧记录与仅携带该字段的其他事件，以及第 6 日历史随日志清理消失 |
...
| F02  | 任务管理  | 初始配置、单条/批量链接、增删任务、校验错误、自动识别 throttle 的 leading/trailing/清空/卸载、手动识别取消 trailing、全局与逐行 `skipFetch` 双向联动及新任务继承、无格式选择器、旧格式子集规范化和固定三格式提交 |
| F03  | 登录状态  | 检查中、有效、失效、IPC 异常和跳转登录页                                                            |
| F04  | 运行日志  | 等待/运行/成功/局部成功/失败、最新 `runId`、自动刷新、损坏行、renderer 会话起点过滤、多个 error/failure 倒序、空状态和清理 |
| F05  | 输出历史  | 最近五日倒序、每本书单一“打开文件夹”入口、局部成功、空状态、打开路径失败和诊断导出                  |
...
| 编号 | 链路          | 必测场景                                                                                                                                                                                                                                                               |
| ---- | ------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| I01  | 初始化        | 临时目录和 SQLite 建立、重复执行、`rebase`、失败路径；最终输出根不预创建旧的顶层 `html/markdown/epub`，版本检查与外网均被替换                                                                                                                                            |
...
| I06  | 输出          | 三种生成模式每次固定 HTML/Markdown/EPUB；最终按书聚合为 `<安全书名>/{html,markdown,epub}`，缓存仍按格式隔离；保留书名、同名重跑、其他书/JSON/诊断不受影响；每书全部多文件与单文件 HTML 都有对应 Markdown，GFM/图片/链接/fallback 可读；缺图或真实写入失败局部成功、Windows 中文目录、超长标题双卷；成品 EPUB 必须可解包，包含必需元数据、正文文件，且 OPF manifest/spine 与 ZIP 条目一致 |
| I07  | 日志汇聚      | 前后端 schema、关联 id、脱敏、按日保留、书籍级 `outputPath` 历史、本会话错误完整筛选（含超过 5000 行、晚打开、刷新失败与清空竞态）、诊断导出和写入失败降级                                                                                                                      |
