...
```text
任务配置 -> 初始化 -> 抓取知乎数据 -> 写入 SQLite -> 读取数据库 -> 排序与分卷 -> 生成 HTML/EPUB
```
...
4. 日志契约和常量只服务当前仓库，唯一来源是 `src/shared/logging/log_contract.ts`，不维护前后端两份魔法字符串。
...
| `src/preload.js`                   | 向主窗口安全暴露 `window.electronAPI`                   |
| `src/public/js-rpc`                | 知乎请求签名所需的独立 renderer 与 preload              |
...
| 页面     | 组件                      | 状态与职责                                                 |
| -------- | ------------------------- | ---------------------------------------------------------- |
| 任务管理 | `component/customer_task` | Ant Design Form、Valtio 状态、登录检查、任务配置和任务启动 |
| 运行日志 | `component/log`           | 阶段状态、最近五日日志、输出历史、诊断导出                 |
...
| 生成       | `src/application/workflow/generate/customer.ts`               | 从 SQLite 组装 Unit、排序、分卷、渲染和输出 |
...
| CLI        | Optique 1.2                                                    |
| 编译与类型 | Babel 8、TypeScript 7                                          |
| 网络       | axios 1.19                                                     |
| 数据库     | SQLite 3 + knex 3                                              |
| 内容生成   | React SSR 模板、HTML、EPUB、sharp 0.35                         |
