---
title: 数据与日志
description: SQLite 数据、结构化日志契约、关联标识与故障诊断。
---
...
`Author.id` 是稳定身份和 `Author` 主键，`Author.url_token` 是规范、可读但可能变化的主页标识。模型层身份解析器先按 `Author.id` 查找、再按 `Author.url_token` 查找，并返回稳定 ID、规范 token 与去重别名；调用方不得假定任务输入一定是 token。原始任务标识继续保留在配置中，以便诊断和兼容历史数据。

`Answer`、`Article`、`Pin` 和 `Author_Ask_Question` 同时具备 `author_id` 与 `author_url_token` 时，关系查询优先稳定 `author_id`，再用规范 token 和身份别名兼容旧记录。`Activity` 当前只有 `url_token`，查询时使用规范 token 与别名集合；本次身份归一化不变更 SQLite schema，也不要求重建既有业务数据库。作者主页、书名和面向用户的日志使用规范 token，token 确实缺失时才显示稳定 ID。
