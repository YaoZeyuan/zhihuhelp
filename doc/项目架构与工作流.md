...
| 入口   | 主要文件                     | 用途                                                 |
| ------ | ---------------------------- | ---------------------------------------------------- |
| 命令行 | `src/ace.ts`、`src/command`  | 直接执行初始化、抓取、生成命令                       |
...
| Electron 调度层 | `src/index.ts`                                             | 创建窗口、管理 cookie、提供 IPC、启动 Ace 命令       |
| 命令层          | `src/ace.ts`、`src/command`                                | 初始化环境、派发抓取、派发生成                       |
| 抓取层          | `src/api/single`、`src/api/batch`                          | 请求知乎接口、分页抓取、调用模型入库                 |
| 数据层          | `src/model`、`src/library/knex.ts`、`src/command/init.sql` | SQLite 表初始化、数据读写、摘要统计                  |
| 生成层          | `src/command/generate`、`src/library/epub`                 | 数据聚合、排序、分卷、HTML 渲染、图片处理、EPUB 打包 |
...
| `runtime.log`                               | 命令执行与 GUI 展示共用的日志文件             |
...
任务配置的核心结构是 `Type_Task_Config`：

| 字段             | 说明                                                              |
| ---------------- | ----------------------------------------------------------------- |
| `fetchTaskList`  | 待抓取任务列表，每项包含任务类型、目标 id、原始输入和是否跳过抓取 |
| `generateConfig` | 生成配置，包含书名、图片质量、分卷数量、排序规则和输出策略        |
| `requestConfig`  | 请求配置，包含 UA 和 cookie                                       |

GUI 会把表单转换为该结构，交给 Electron 主进程。Electron 会更新 cookie，然后写入 `config.json`。命令行模式也读取同一份 `config.json`。
...
sequenceDiagram
  ...
  Main->>Ace: generate:manifest
  Main->>Ace: Init:Env
  Ace->>DB: 建表 / 初始化目录
  Main->>Ace: Fetch:Customer
  Ace->>DB: 抓取并写入原始数据
  Main->>Ace: Generate:Customer
  Ace->>DB: 读取数据
  Ace->>FS: 输出 HTML / EPUB
  Main->>FS: 打开输出目录
```
...
命令行入口是 `src/ace.ts`。它会执行三个动作：

1.  根据 `src/adonisrc.json` 生成 `.adonisrc.json`。
2.  调用 Ace 的 `generate:manifest`。
3.  默认执行 `Init:Env`，然后执行用户传入的命令。
...
| `pnpm ace Init:Env --rebase` | 删除当前数据库并重建表      |
| `pnpm ace Fetch:Customer`    | 按 `config.json` 抓取数据   |
| `pnpm ace Generate:Customer` | 按 `config.json` 生成电子书 |

## 初始化流程

`src/command/init_env.ts` 负责初始化：

1.  调用升级接口检查新版本。
2.  创建 `PathConfig.allPathList` 中定义的缓存和输出目录。
3.  如果带 `--rebase`，删除当前版本 SQLite 文件。
4.  执行 `src/command/init.sql` 建表。

数据库文件由 `src/config/common.ts` 中的 `db_version` 决定，目前为 `zhihu_v1.0.2.sqlite`。

## 抓取工作流程

抓取入口是 `src/command/fetch/customer.ts`。

```mermaid
flowchart TD
  A["读取 config.json"] --> B["按任务类型合并 id"]
...
生成入口是 `src/command/generate/customer.ts`。
