...
| 层级             | 默认联网 | 运行环境             | 目标                                                        |
| ---------------- | -------- | -------------------- | ----------------------------------------------------------- |
| 单元测试         | 否       | Node                 | 配置、URL、任务分派、排序分卷、日志契约与脱敏等纯逻辑       |
...
4. 在线 runner 的日志遵循[数据与日志](数据与日志.md)中的同一套脱敏规则。
...
| 编号 | 模块 | 必测场景 |
| ---- | ---- | -------- |
| U01 | 配置 | 默认配置创建、当前 schema 读写、缺失/非法字段、空文件、损坏 JSON；旧 schema 明确拒绝并给出诊断，不做隐式迁移 |
...
| U11 | HTML/EPUB | 特殊字符、空内容、图片三种策略、安全文件名、长标题卷名、目录页、单页 HTML，以及 EPUB XML 转义、图片 MIME 与封面清单 |
| U12 | 数据模型 | 主表、关系表、损坏 `raw_json` 和问题信息从回答记录恢复 |
| U13 | 输出历史 | 仅扫描最近 5 个自然日中 `success/partial_success` 的 `output.created`、保留告警状态、规范化路径去重、倒序、损坏行容错、忽略仅携带 `outputPath` 的其他成功事件，以及第 6 日历史随日志清理消失 |
...
| I06 | 输出 | 三种生成模式、HTML/EPUB、图片请求 mock、缺图局部成功、Windows 中文目录、超长标题真实双卷、EPUB 包结构和最终文件可读取 |
...
`pnpm test:online` 是显式命令：Cookie 前置验证失败必须非零退出；仅已标注的外部不稳定场景可以 `SKIP`，并在摘要中给出原因。`partial_success` 不能被测试当作 `success`，必须断言成功数、失败数和失败实体摘要。
