...
`pnpm test` 强制断网并使用临时目录，不读取根目录 `config.json`。在线两条命令只读 `config.json.request.cookie`；Cookie 缺失、为空或失效会提示手工更新并以非零状态退出。完整规则、公开测试输入和 `KEEP_TEST_ARTIFACTS=1` 用法见[测试与 Fixture](测试与Fixture.md)。
...
5. 前后端按日日志写入 `PathConfig.rootPath/log`，文件名和排查方法见[数据与日志](数据与日志.md)。
