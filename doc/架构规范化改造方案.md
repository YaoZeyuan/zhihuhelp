...
1.  知乎接口和签名逻辑可能变化，抓取层改造时要避免同时大幅改签名算法。
    1.  @todo(codex) 答：知乎接口和签名逻辑固定，已经3年没有变化，改造时只改项目本身代码，但不要变动知乎接口的响应值数据和签名算法
2.  SQLite 中已有 `raw_json` 数据，若修改表结构，需要明确迁移或重建策略。
    1.  @todo(codex) 答：允许修改表结构，以便更好的读取数据内容。目前sqlite数据库中有存储的样例数据，你可以根据sqlite中的数据，提取出对应json，生成数据类型定义。或者根据D:\win_www\zhihuhelp\src\type 中的类型生成也可以
1.  Electron、sqlite3、sharp 都涉及原生依赖，打包流程应单独验证。
2.  允许不兼容旧命令和目录后，文档必须同步，否则使用体验会断裂。
    1.  @todo(codex) 答：可以根据工程美感，调整命令行指令以及使用方式
...
4.  是否优先保证 GUI 入口，还是优先保证命令行入口。
    1.  @todo(codex) 答：优先保障命令行入口
5.  是否在改造过程中保留旧入口一段时间作为过渡。
    1.  @todo(codex) 答：不需要，目标是借助这次改造，推进成为完美项目
