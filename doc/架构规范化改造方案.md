...
| 当前文件                                           | 当前问题                                                                     |
| -------------------------------------------------- | ---------------------------------------------------------------------------- |
| `src/command/generate/customer.ts`                 | 读取配置、查库、组装、排序、分卷、调用输出全部混在一起                       |
| `src/command/generate/resource/library/package.ts` | 同时是领域模型、排序工具、分卷辅助                                           |
| `src/command/generate/library/epub_generator.ts`   | 同时负责 HTML 清洗、图片替换、图片下载、Latex 转换、静态资源、EPUB、输出复制 |
...
`src/index.ts` 同时处理窗口、cookie、IPC、配置、Ace 命令、日志裁剪等。
...
5.  `src/ace.ts`、`src/index.ts`。
6.  `src/command/fetch/customer.ts`、`src/command/generate/customer.ts`。
