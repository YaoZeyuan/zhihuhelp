...
class Activity extends Base {
   ...
   * @param sortBy
   */
  static async asyncGetAutherActivityList(url_token: string, afterTimeAt: number = 0, limit: number = 20): Promise<Array<TypeActivity.Record>> {
  ...
  */
  static async asyncGetAutherLastActivityAt(url_token: string): Promise<number> {
    ...
    const lastActivityAt = _.get(record, ['data', 0, 'id'], now)
